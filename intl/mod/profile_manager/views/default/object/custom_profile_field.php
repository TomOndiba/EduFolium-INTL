<?php 
	...
	*/
	...
	if(!empty($vars['entity']->metadata_label)){
		$extra_info .= elgg_echo("profile_manager:admin:metadata_label") . ": " . $vars['entity']->metadata_label;
	} else {
		if(elgg_echo("profile:" . $vars['entity']->metadata_name) == "profile:" . $vars['entity']->metadata_name){
			$extra_info .= elgg_echo("profile_manager:admin:metadata_label_untranslated") . ": <i>" . elgg_echo("profile:" . $vars['entity']->metadata_name) . "</i>";
		} else {
			$extra_info .= elgg_echo("profile_manager:admin:metadata_label_translated") . ": " . elgg_echo("profile:" . $vars['entity']->metadata_name) . "";
		}
	}
	
	// options
	if(!empty($vars['entity']->metadata_options)){
		$extra_info .= elgg_echo("profile_manager:admin:metadata_options") . ": " . $vars['entity']->metadata_options . "";
	}
	
	// Hint
	if(!empty($vars['entity']->metadata_hint)){
		$extra_info .= elgg_echo("profile_manager:admin:metadata_hint") . ": " . $vars['entity']->metadata_hint . "";
	}
